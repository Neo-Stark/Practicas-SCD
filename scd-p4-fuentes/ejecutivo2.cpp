...
int main( int argc, char *argv[] )
{
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
        ...
        // Pregunta 1:
        // El tiempo minimo de espera es 10 ms 
        // Pregunta 2:
        // En un sistema real, donde suponemos retrasos propios del sistema no seria planificable
         ...
         sleep_until( ini_sec );
         time_point<steady_clock> ciclo_fin = steady_clock::now();
         steady_clock::duration duracion = ciclo_fin - ini_sec;
         float tiempo = milliseconds_f(duracion).count();
         cout << "Diferencia en milisegundos: " <<  tiempo << endl;
          if ( tiempo > 10)
          {
              cout << "ERROR: limite superado. Abortando Ejecucion" << endl;
              exit(EXIT_FAILURE);
          }
      }
   }
